<template>
  ...
  <div class="container">
    ...
    <p v-else>⚠️</p>
    <l-map style="height: 888px;" :center="center" :zoom="parseInt(zoom
  )" @update:center="$emit('update-center',$event)" @update:zoom="$emit('update-zoom',$event)" @update:bounds="$emit('update-bounds',$event)">
...
// const PROPS = defineProps({ basemaps: Array, subs: Array, region: String, basemap: String, center: String, zoom: Number, brookline: Object, brighton: Object, traceActive: Object, brooklinePoly: Object, brightonPoly: Object, styles: Object });
...
const lll = () => {
  let o = (PROPS.traceActive.features && PROPS.traceActive.features[0].geometry.coordinates.length > 0) ? PROPS.traceActive.features[0].geometry.coordinates[PROPS.traceActive.features[0].geometry.coordinates.length - 1] : [0, 0]
  return o ? [o[1], o[0]] : [];
}
